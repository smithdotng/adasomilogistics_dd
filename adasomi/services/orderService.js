...
async function fundEscrow(order) {
    ...
    order.timeline.push({ status: 'awaiting_assignment', note: 'Escrow funded. Awaiting rider assignment.' });
    await order.save();
    return order;
}

async function verifyPickupOtp(order, code) {
    if (order.otp.pickupCode !== code) {
        throw new Error('Incorrect pickup OTP.');
    }
    order.otp.pickupVerifiedAt = new Date();
    order.status = 'picked_up';
    order.timeline.push({ status: 'picked_up', note: 'Pickup OTP verified by sender.' });
    await order.save();
    return order;
}
...
async function verifyDeliveryPin(order, pin) {
    if (order.otp.deliveryPin !== pin) {
        throw new Error('Incorrect delivery PIN.');
    }
    order.otp.deliveryVerifiedAt = new Date();
    order.status = 'delivered';
    order.timeline.push({ status: 'delivered', note: 'Delivery PIN verified by recipient.' });
    await settlePayout(order);
    order.status = 'completed';
    order.escrow.status = 'released';
    order.escrow.releasedAt = new Date();
    order.timeline.push({ status: 'completed', note: 'Escrow released. Payouts settled.' });
    await order.save();
    return order;
}

async function settlePayout(order) {
...
module.exports = { fundEscrow, verifyPickupOtp, verifyDeliveryPin, settlePayout };

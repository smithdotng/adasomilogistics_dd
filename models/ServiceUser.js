...
const userSchema = new mongoose.Schema({
    ...
    address: {
        street: String,
        city: String,
        county: String,  // County field for UK addresses
        postcode: String,
        country: { type: String, default: 'UK' }
    },
    
    // Multi-tenant relationship - Every user belongs to a care provider
    careProviderId: {
    ...
    // Service Care Provider specific fields
    careProviderInfo: {
        companyName: String,
        ...
        icoNumber: String // ICO registration number
    },
    
    // Support Worker (Carer) specific fields
    supportWorkerInfo: {
    ...
    // Service User (Patient) specific fields - COMPLETE SERVICE USER INFO
    serviceUserInfo: {
        nhsNumber: String,
        dateOfBirth: Date,
        gender: String,
        maritalStatus: String,  // Marital status field
...
userSchema.index({ 'supportWorkerInfo.employeeId': 1 });
...
// Virtual for age (if service user)
userSchema.virtual('age').get(function() {
    if (this.role === 'service_user' && this.serviceUserInfo?.dateOfBirth) {
        const today = new Date();
        const birthDate = new Date(this.serviceUserInfo.dateOfBirth);

...
// Usage: tsx seed/createAdmin.ts "First" "Last" "email@example.com" "08012345678" "yourPassword"
const [firstName, lastName, email, phone, password] = process.argv.slice(2);

async function run() {
    if (!email || !password) {
        console.log('Usage: tsx seed/createAdmin.ts "First" "Last" "email@example.com" "phone" "password"');
        console.log('Example: tsx seed/createAdmin.ts Jane Doe jane@adasomilogistics.com 08012345678 SuperSecret123');
        process.exit(1);
    }
